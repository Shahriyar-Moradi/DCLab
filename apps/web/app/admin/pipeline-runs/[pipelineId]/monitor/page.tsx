...
import { GlassPanel, MetricCard as ProductMetricCard, ProductPageHeader } from "@/app/components/product/ProductPrimitives";
...
export default function PipelineMonitorPage() {
  ...
  const business = object(hierarchy.business); const domain = object(hierarchy.domain); const workflow = object(hierarchy.workflow); const workflowRun = object(hierarchy.workflow_run);
  ...
  return <div className="pb-16">
    <ProductPageHeader
      breadcrumbs={[
        { label: businessId ? "Business administration" : "Businesses", href: root },
        { label: text(business.name), href: businessBase },
        { label: text(domain.name) },
        { label: text(workflow.name) },
        { label: "Workflow Run", href: `${businessBase}/workflow-runs/${text(workflowRun.id)}` },
    ...
    {capabilities.openai_pipeline_audit ? <Panel title="OpenAI Auditor" subtitle="OPENAI AUDIT = ADVISORY · kept separate from semantic decisions and authoritative deterministic checks."><LlmTable rows={audit} /><div className="mt-4"><Button disabled={!canDeepAudit || deepAudit.isPending} onClick={() => { if (!sourceRunId) return; deepAudit.mutate({ businessId, runId: sourceRunId }, { onSuccess: () => void query.refetch() }); }}>{deepAudit.isPending ? "Running deep audit…" : "Run deep audit"}</Button>{!canDeepAudit ? <p className="mt-2 text-body text-ink-muted">{businessId ? "Deep audit requires Business Admin plus the deep_audit capability." : "Deep audit requires DCLab Admin."}</p> : null}{deepAudit.isError ? <p className="mt-2 text-body text-oxblood">Deep audit could not be completed.</p> : null}</div></Panel> : <CapabilityUnavailable name="openai_pipeline_audit" />}
...
function Panel({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) { return <GlassPanel className="mt-6" title={title} description={subtitle}>{children}</GlassPanel>; }
function Metric({ label, value }: { label: string; value: string }) { return <ProductMetricCard label={label} value={value} />; }
function Flow({ title, steps, footer }: { title: string; steps: string[]; footer?: string }) { return <div className="rounded-lg border border-hairline bg-navy-soft/40 p-5"><p className="text-eyebrow uppercase text-ink-muted">{title}</p><div className="mt-4 flex flex-wrap items-center gap-3">{steps.map((step, index) => <span key={step} className="contents"><span className="rounded bg-navy px-3 py-2 font-mono text-data text-white">{step}</span>{index < steps.length - 1 ? <span>→</span> : null}</span>)}</div>{footer ? <p className="mt-4 font-mono text-data text-ink-muted">{footer}</p> : null}</div>; }
...
function Empty({ children }: { children: ReactNode }) { return <p className="mt-4 rounded-lg border border-hairline bg-navy-soft/40 p-4 text-body text-ink-muted">{children}</p>; }
function CapabilityUnavailable({ name }: { name: string }) { return <section className="mt-6 rounded-lg border border-hairline bg-paper-raised p-5"><p className="font-mono text-data text-ink-muted">{name} is not enabled for this workspace.</p></section>; }
