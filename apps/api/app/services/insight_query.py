"""Reads the latest workspace-owned simulation run per use case and translates
it into client-facing insights, grouped by business function.

Unowned (workspace_id IS NULL) historical rows are archive only and never
appear here (ADR 0004). Admin writes go through `/admin/simulations/run` with
the current workspace. Client Labs is a separate workspace-scoped trial path.
"""
...
) -> dict[InsightCategory, list[ClientFacingInsight]]:
    grouped: dict[InsightCategory, list[ClientFacingInsight]] = {
        category: [] for category in InsightCategory
    }
    for use_case_name, run in latest_runs_by_use_case(db, workspace_id).items():
