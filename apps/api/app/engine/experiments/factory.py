"""M1-compatible factory trainer.

Extracted from the original conversion-layer train loop so opportunity
conversion and simulation keep the same artifacts and metrics.
"""
...
def load_labeled_frame(path: Path, target: str = "converted") -> pd.DataFrame:
...
def opportunity_feature_table(frame: pd.DataFrame, target: str) -> tuple[pd.DataFrame, np.ndarray]:
    rows = []
    labels = []
...
def generic_feature_table(frame: pd.DataFrame, config: dict, target: str) -> tuple[pd.DataFrame, np.ndarray]:
...
def feature_table(frame: pd.DataFrame, config: dict, target: str) -> tuple[pd.DataFrame, np.ndarray]:
    if config.get("kind") == "simulation":
        return generic_feature_table(frame, config, target)
    return opportunity_feature_table(frame, target)


def evaluate_binary(y_true, probability) -> dict[str, float]:
    y_true = list(y_true)
...
def split_frame(frame: pd.DataFrame, target: str = "converted"):
...
def slice_matrix(table: pd.DataFrame, spec: CandidateSpec) -> list[list[float]]:
    records = table.to_dict(orient="records")
    return [feature_vector(record, spec.features) for record in records]


def run_factory_train(
    ...
    frame = load_labeled_frame(csv_path, target)
    train_df, test_df, split_kind = split_frame(frame, target)
    x_train, y_train = feature_table(train_df, config, target)
    x_test, y_test = feature_table(test_df, config, target)
    ...
    for spec in specs:
        ...
        train_x = slice_matrix(x_train, spec)
        test_x = slice_matrix(x_test, spec)
        model.fit(train_x, y_train)
        proba = model.predict_proba(test_x)[:, 1]
        score = evaluate_binary(y_test, proba)
    ...
    blend_metrics = evaluate_binary(y_test, blended)
    ...
    member_meta = []
    persist_ids = selected_ids if fusion == "weighted_blend" else [best_single_id]
    ...
    serving = {"fusion": fusion, "members": member_meta, "weights": weights}
